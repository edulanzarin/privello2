// Feature: privello-platform, Property 8: Unicidade de email é case-insensitive
/**
 ...
 *
 *   For any dois cadastros com emails `a` e `b` tais que
 *   `a.toLowerCase() === b.toLowerCase()`, no máximo um deles termina em
 *   sucesso; o segundo recebe `{ ok: false, reason: "EMAIL_EM_USO" }`.
 *
 * Estratégia do teste (espelha Property 9 para identificador):
 *
 *   - Cada iteração começa com um email base **fresco**, derivado de
 *     `fc.uuid()`, na forma `u<hex32>@example.com`. Isso garante zero
 *     interferência entre iterações mesmo que o reset do store falhe
 *     (defesa em profundidade), porque dois UUIDs sorteados em
 *     iterações diferentes têm probabilidade desprezível de colidir.
 *   - Duas permutações de caixa independentes do mesmo base
 *     (`permA`, `permB`) são geradas: por construção
 *     `permA.toLowerCase() === permB.toLowerCase()`. Caracteres não
 *     alfabéticos (dígitos, `@`, `.`) permanecem inalterados, então a
 *     regex de email (`/^[^\s@]+@[^\s@]+\.[^\s@]+$/`) continua sendo
 *     satisfeita.
 *   - Os dois cadastros recebem **identificadores DIFERENTES** (com
 *     prefixos disjuntos `i...` e `j...`), garantindo que a única
 *     colisão possível entre as duas inscrições seja o email. Isso
 *     blinda o teste contra regressões em `detectarColisao` que
 *     reportassem `IDENTIFICADOR_EM_USO` quando a colisão real é de
 *     email.
 *   - Roda `registrar` duas vezes em sequência. A propriedade exige:
 *       1. Exatamente um dos dois resultados é `{ ok: true, ... }`.
 *       2. O outro é `{ ok: false, reason: "EMAIL_EM_USO" }`.
 *
 ...
 *     `tx.session.create`. O mesmo padrão de
 *     `tests/property/registrar-roundtrip.test.ts` é usado: `vi.hoisted`
 *     para os stores + `vi.mock("@/lib/db", ...)` na fábrica.
...
import { validNomeArb, validSenhaArb } from "./generators";
    ...
    const txClient = {
        user: {
            ...
            create: async (args: {
                ...
                // criar dois usuários com o mesmo email ou
                // identificador — falhar loud em vez de mascarar.
...
/**
 * Constrói um email base válido a partir de um `fc.uuid()`. Removendo
 * os hífens, ficamos com 32 caracteres hexadecimais; prefixamos com
 * `u` para garantir que a parte local começa com letra (sem violar a
 * regex de email) e completamos com `@example.com`. Comprimento total
 * em caixa baixa: 1 + 32 + 12 = 45 caracteres, dentro de [5, 254].
 *
 * O email retornado está em caixa baixa por convenção; as variantes
 * mistas são produzidas pelo {@link casePermuteArb}.
 */
const baseEmailArb: fc.Arbitrary<string> = fc.uuid().map((u) => {
    const hex = u.replace(/-/g, "");
    return `u${hex}@example.com`;
});

/**
 * Permuta independentemente a caixa de cada caractere alfabético de
 * `base`. Caracteres sem variante de caixa (dígitos, `@`, `.`) ficam
 * inalterados, então o resultado continua casando com a regex de email
 * `/^[^\s@]+@[^\s@]+\.[^\s@]+$/` (Requirement 2.7).
 *
...
/**
 * Gera dois identificadores válidos, **distintos mesmo após
 * normalização para caixa baixa**, e com prefixos disjuntos (`i...` e
 * `j...`) para garantir que a única colisão entre os dois cadastros é
 * de email. Cada identificador usa um UUID independente como semente,
 * limitado a 30 caracteres (limite máximo do Requirement 2.5).
 */
const distinctIdentificadoresArb: fc.Arbitrary<[string, string]> = fc
    .tuple(fc.uuid(), fc.uuid())
    .map(([ua, ub]) => {
        const a = `i${ua.replace(/-/g, "")}`.slice(0, 30);
        const b = `j${ub.replace(/-/g, "")}`.slice(0, 30);
        return [a, b] as [string, string];
    });

/**
 * Cenário completo de uma rodada: email base + duas permutações de
 * caixa do mesmo email + dois identificadores distintos + nomes/senhas
 * válidos para cada cadastro.
 */
const scenarioArb = baseEmailArb.chain((base) =>
    fc.record({
        permA: casePermuteArb(base),
        permB: casePermuteArb(base),
        identificadores: distinctIdentificadoresArb,
...
describe("Property 8: Unicidade de email é case-insensitive", () => {
    beforeEach(() => {
    ...
    it(
        "for any two registrations with emails a, b such that a.toLowerCase()===b.toLowerCase(), at most one succeeds and the second receives EMAIL_EM_USO",
        { timeout: 60_000 },
        async () => {
            await fc.assert(
                fc.asyncProperty(scenarioArb, async (scenario) => {
                    ...
                    const [identA, identB] = scenario.identificadores;
                    const [nomeA, nomeB] = scenario.nomes;
                    const [senhaA, senhaB] = scenario.senhas;

                    const resA = await registrar({
                        nome: nomeA,
                        email: scenario.permA,
                        identificador: identA,
                        senha: senhaA,
                    });

                    const resB = await registrar({
                        nome: nomeB,
                        email: scenario.permB,
                        identificador: identB,
                        senha: senhaB,
                    });
                    ...
                    // { ok: false, reason: "EMAIL_EM_USO" }.
                    const falhas = [resA, resB].filter(
                        (r): r is Extract<typeof r, { ok: false }> => !r.ok,
                    );
                    expect(falhas.length).toBe(1);
                    expect(falhas[0].reason).toBe("EMAIL_EM_USO");

                    // Como os dois identificadores são distintos por
                    // construção (prefixos disjuntos `i...`/`j...`), a
                    // colisão NÃO pode ser de identificador. Esta
                    // asserção blinda contra regressões em
                    // `detectarColisao` que confundissem a ordem das
                    // verificações.
                    expect(falhas[0].reason).not.toBe("IDENTIFICADOR_EM_USO");
                }),
                { numRuns: 30 },
            );
        },
