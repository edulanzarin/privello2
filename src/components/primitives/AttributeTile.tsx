...
export function AttributeTile({
    ...
    const composed = [
        "flex flex-col items-center gap-2.5 rounded-2xl bg-neutral-50 px-3 py-5 text-center transition-all duration-200 hover:bg-accent-soft/60 hover:-translate-y-0.5",
        className ?? "",
    ]
        .filter(Boolean)
        .join(" ");
    ...
    return (
        <div className={composed}>
            <div
                aria-hidden="true"
                className="flex h-11 w-11 items-center justify-center rounded-full bg-accent-soft text-accent-deep"
            >
                {icon}
            </div>
