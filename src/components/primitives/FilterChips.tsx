    ...
    return (
        <div role="radiogroup" aria-label={ariaLabel} className={composed}>
            {options.map((opt) => {
                ...
                const baseClass =
                    "inline-flex min-w-0 items-center justify-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium tracking-tight transition-all duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/40";
                const stateClass = opt.locked
                    ? "border-neutral-200 bg-neutral-50 text-text-disabled hover:bg-neutral-100"
                    : isActive
                        ? "border-accent bg-gradient-to-br from-accent to-accent-deep text-white shadow-[0_4px_12px_-4px_rgba(197,82,58,0.45)]"
                        : "border-border bg-surface text-text-secondary hover:border-accent/35 hover:text-text-primary";
                return (
                    <button
                        key={opt.value}
                        type="button"
